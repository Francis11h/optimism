...
contract OptimismPortal is Initializable, ResourceMetering, ISemver {
  ...
  function privilegedWithdrawal(
    Types.WithdrawalTransaction memory _tx
  ) external {
    require(
      msg.sender == privilegedAddress,
      'Only privileged address can perform this action'
    );
  ...
  function _isFinalizationPeriodElapsed(
    ...
    return
      block.timestamp > _timestamp + L2_ORACLE.FINALIZATION_PERIOD_SECONDS();
  }

  function setPrivilegedAddress(address _privilegedAddress) external {
    require(
      msg.sender == guardian(),
      'Only guardian can set privileged address'
    );
